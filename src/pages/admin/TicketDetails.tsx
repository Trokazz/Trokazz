...
import { Send, ArrowLeft, CheckCircle, Clock, XCircle, MessageSquare, Bug, Lightbulb, HelpCircle } from "lucide-react";
...
const fetchTicketDetails = async (ticketId: string): Promise<SupportTicketQueryResult | null> => {
...
const TicketDetails = () => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          ...
          queryClient.invalidateQueries({ queryKey: ["ticketDetails", ticketId] }); // To update last_user_reply_at/last_admin_reply_at
          queryClient.invalidateQueries({ queryKey: ["supportTickets"] }); // For the main list
        }
      )
      .subscribe();
  ...
  const handleSendMessage = async () => {
    ...
    const { data: profileData, error: profileError } = await supabase.from('profiles').select('role').eq('id', currentUser.id).single();
    ...
    try {
      ...
      const updatePayload: { last_admin_reply_at?: string; last_user_reply_at?: string; status?: string } = {};
      if (isAdmin) {
        updatePayload.last_admin_reply_at = new Date().toISOString();
        if (ticket?.status === 'new') { // Automatically set to in_progress if new
  ...
  const handleStatusChange = async (newStatus: 'new' | 'in_progress' | 'resolved' | 'closed') => {
    ...
    try {
      const { data: profileData, error: profileError } = await supabase.from('profiles').select('full_name, role').eq('id', currentUser.id).single();
      ...
      // Insert an automated message into the chat history
      let statusMessage = '';
      switch (newStatus) {
        case 'in_progress': statusMessage = `O status do ticket foi alterado para 'Em Progresso' por ${adminName}.`; break;
        case 'resolved': statusMessage = `O status do ticket foi alterado para 'Resolvido' por ${adminName}.`; break;
        case 'closed': statusMessage = `O status do ticket foi alterado para 'Fechado' por ${adminName}.`; break;
        case 'new': statusMessage = `O status do ticket foi alterado para 'Novo' por ${adminName}.`; break;
      }

      if (statusMessage) {
        const { error: messageInsertError } = await supabase.from("ticket_messages").insert({
          ticket_id: ticketId,
          sender_id: currentUser.id, // Admin is the sender of this system message
          content: statusMessage,
          is_admin_message: true, // Mark as admin message
        });
        if (messageInsertError) {
          console.error("Error inserting status change message:", messageInsertError);
          // Don't throw, as this shouldn't block the status update
        }
      }
  ...
  if (isTicketError || !ticket) {
    return <div className="text-center py-10 text-red-500">Erro: {ticketError?.message || "Ticket não encontrado."}</div>;
  }
  if (isMessagesError) {
      ...
      <div className="flex items-center p-3 border-b">
        ...
        <div className="flex-grow">
          ...
          <CardDescription className="text-sm">
            De: {ticket.profiles?.full_name || ticket.profiles?.email || 'Usuário Desconhecido'}
          </CardDescription>
        </div>
        ...
        {allMessages.map((msg) => (
          <div
            ...
            className={cn(
              ...
              msg.is_admin_message ? "justify-end" : "justify-start"
            )}
          >
            ...
            {msg.is_admin_message && (
              <Avatar className="h-8 w-8">
                ...
                <AvatarFallback>{currentUser?.user_metadata?.full_name?.[0] || 'A'}</AvatarFallback>
              </Avatar>
            )}
          </div>
        ))}
