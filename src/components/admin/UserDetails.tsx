...
import { safeFormatDate, safeFormatDistanceToNow, getOptimizedImageUrl } from "@/lib/utils"; // Importar getOptimizedImageUrl
...
const fetchUserDetailsAndActivity = async (userId: string) => {
  ...
  // NOVO: Busca o histórico de transações de crédito
  const { data: creditTransactions, error: creditTransactionsError } = await supabase
    .from("credit_transactions")
    .select("*, advertisements(title)") // Join with advertisements to get ad title for related_ad_id
    .eq("user_id", userId)
    .order("created_at", { ascending: false });
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="violations">
        <TabsList className="grid w-full grid-cols-2 md:grid-cols-7"> {/* Aumentado para 7 colunas */}
          ...
          <TabsTrigger value="credit-transactions">Créditos</TabsTrigger> {/* Nova aba */}
        </TabsList>
        ...
        <TabsContent value="credit-transactions"> {/* Conteúdo da nova aba */}
          <Card>
